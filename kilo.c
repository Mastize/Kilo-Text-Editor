...
int editorRowRxToCx(erow *row, int rx) {
    ...
    // this would return an out of bound cx
    // which should not be executed normally
    return cx;
}
...
void editorDelRow(int at) {
    ...
    for (int j = at + 1; j < E.numrows - 1; j++) {
        E.row[j].idx--;
    }
    E.numrows--;
    E.dirty++;
...
void editorRowRelChar(erow *row, int at) {
...
void editorRefreshScreen() {
    ...
    // \x1b is 27 (ESC)
    // escape sequence start with '27' and '['
